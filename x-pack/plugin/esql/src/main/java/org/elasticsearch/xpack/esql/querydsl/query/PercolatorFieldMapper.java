...
/**
 * Duplicate of PercolatorFieldMapper from percolator module, placed in ESQL plugin
 * to avoid classloader issues. Uses local QueryStore interface from PercolateQueryBuilder.
 */
public class PercolatorFieldMapper extends FieldMapper {
    ...
    public static class PercolatorFieldType extends MappedFieldType {
        ...
        public MappedFieldType queryBuilderField;
        MappedFieldType minimumShouldMatchField;

        RangeFieldMapper.RangeFieldType rangeField;
        public boolean mapUnmappedFieldsAsText;

        // Original field type from percolator module (for delegation)
        private final MappedFieldType originalFieldType;
        ...
        /**
         * Constructor that wraps an original PercolatorFieldType from the percolator module.
         * This allows us to work with field types from different classloaders.
         * We extract the data we need from public sources (IndexSettings, MappingLookup) instead of reflection.
         */
        public PercolatorFieldType(MappedFieldType original, SearchExecutionContext context) {
            super(original.name(), original.indexType(), original.isStored(), original.meta());
            this.originalFieldType = null; // Don't need to keep reference since we'll implement percolateQuery ourselves

            // Get mapUnmappedFieldsAsText from index settings (no reflection needed)
            this.mapUnmappedFieldsAsText = INDEX_MAP_UNMAPPED_FIELDS_AS_TEXT_SETTING.get(context.getIndexSettings().getSettings());

            // Get all sub-fields by looking them up from the mapping (no reflection needed)
            // These are stored as sub-fields: {percolator_field_name}.{sub_field_name}
            String baseName = original.name();
            this.queryTermsField = context.getFieldType(baseName + ".extracted_terms");
            this.extractionResultField = context.getFieldType(baseName + ".extraction_result");
            this.queryBuilderField = context.getFieldType(baseName + ".query_builder_field");
            this.rangeField = (RangeFieldMapper.RangeFieldType) context.getFieldType(baseName + ".range_field");
            this.minimumShouldMatchField = context.getFieldType(baseName + ".minimum_should_match_field");

            if (this.queryTermsField == null
                || this.extractionResultField == null
                || this.queryBuilderField == null
                || this.rangeField == null
                || this.minimumShouldMatchField == null) {
                throw new IllegalStateException("Could not find all required sub-fields for percolator field [" + original.name() + "]");
            }
        }

        // Public accessors that use our cached values (no reflection needed)
        public boolean getMapUnmappedFieldsAsText() {
            return mapUnmappedFieldsAsText;
        }

        public MappedFieldType getQueryBuilderField() {
        ...
        Query percolateQuery(
            String name,
            PercolateQueryBuilder.QueryStore queryStore,
            ...
            // Use our own implementation - all fields are already looked up from the mapping
            ...
            // Create PercolateQuery directly - we have our own copy in ESQL
    ...
    public static SearchExecutionContext configureContext(SearchExecutionContext context, boolean mapUnmappedFieldsAsString) {
