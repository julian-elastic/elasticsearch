...
/**
 * Duplicate of PercolateQueryBuilder from percolator module, placed in ESQL plugin
 * to avoid classloader issues. Now uses direct access to percolator module classes.
 */
public class PercolateQueryBuilder extends AbstractQueryBuilder<PercolateQueryBuilder> {

    /**
     * Local interface matching PercolateQuery.QueryStore since PercolateQuery is package-private.
     * This interface matches the signature of the percolator module's QueryStore.
     */
    @FunctionalInterface
    ...
    protected Query doToQuery(SearchExecutionContext context) throws IOException {
        ...
        // The SearchExecutionContext wrapper in AbstractLookupService ensures we get our PercolatorFieldType
        ...
        // Now we can access fields via getters that delegate to original if needed
        String queryName = this.name != null ? this.name : pft.name();
        boolean mapUnmappedFieldsAsText = pft.getMapUnmappedFieldsAsText();
        MappedFieldType queryBuilderField = pft.getQueryBuilderField();

        SearchExecutionContext percolateShardContext = wrap(context);
        percolateShardContext = PercolatorFieldMapper.configureContext(percolateShardContext, mapUnmappedFieldsAsText);
        QueryStore queryStore = createStore(queryBuilderField, percolateShardContext);

        // Call percolateQuery method directly - pass context for error handling
    ...
    static QueryStore createStore(MappedFieldType queryBuilderFieldType, SearchExecutionContext context) {
