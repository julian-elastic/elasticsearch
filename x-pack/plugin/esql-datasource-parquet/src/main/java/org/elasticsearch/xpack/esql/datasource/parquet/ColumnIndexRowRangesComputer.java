...
final class ColumnIndexRowRangesComputer implements FilterPredicate.Visitor<RowRanges> {
    ...
    private <T extends Comparable<T>> RowRanges evaluateLeaf(Operators.Column<T> column, PagePredicate<T> predicate) {
        ...
        // Float16) instead of misreading the raw bytes. If the column is not present in the
        // schema (e.g. legacy callers that constructed metadata without a schema), fall back to
        // keeping all rows so the page is rechecked per-row by FilterExec.
        PrimitiveType primitive = resolvePrimitiveType(metadata.schema(), columnPath);
        if (primitive == null) {
            return all();
        }
    ...
    /**
     * Resolves the file-level {@link PrimitiveType} for a dotted column path against the captured
     * {@link MessageType}. Returns {@code null} when the column is not present (e.g. older metadata
     * built without a schema, or a path that does not resolve in the file). Iterating the columns
     * list rather than relying on {@code MessageType#containsField} keeps the lookup robust against
     * nested columns whose dotted path is not a top-level field name.
     */
    private static PrimitiveType resolvePrimitiveType(MessageType schema, String columnPath) {
        for (ColumnDescriptor cd : schema.getColumns()) {
            if (String.join(".", cd.getPath()).equals(columnPath)) {
                return cd.getPrimitiveType();
            }
        }
        return null;
    }
